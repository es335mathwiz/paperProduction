...
compute completely analytic solutions for each of these phases.
...
Suppose, for now, that we are interested in computing homogeneous soltions -- $\Psi=0$:
...
representation for the equation system~\ref{eq:homo}. The method is an extension
...
Numerical implementations written in Python,  Mathematica,  Matlab,  and  ``C'' are available from the author.\footnote{\url{http://www.federalreserve.gov/pubs/oss/oss4/aimindex.html} and via gitHub \url{https://github.com/es335mathwiz}.}   
Numerical implementation are available in Dynare and in Troll.
