...
{\small
Given a linear reference  model, $\linMod$, of dimension $\numX$,
...
{\small
  \begin{algorithm}[H]{For each $(\xtmEpsArg) \in \thePts $, improve the \ADR\ $\xzFunc^k(\xtmEpsArg)$}
...
\Output{$\xzFunc^{k+1}(\xtmEpsArg)=\xnxt{t}$}
