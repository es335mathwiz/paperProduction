...
compute $\xzFuncGuess(\xtArg;\xgusst{t})$\;
...
& 
\mathcal{I}_t 
\arrow [ u, "\proj{x_{t-1}}"right ,near start ]   
\arrow [ lu,"\proj{\epsilon_t}" above] 
\arrow[r,"\xzFuncGuess(\xgusst{t})"{name=U,below}]
\arrow [ l,"\proj{z_t}" ] 
%\arrow [rr "huh" ]
& 
\eqnArg 
...
\arrow[ u,"\proj{x_{t}}"]
\arrow[ ru,"\proj{\expct{x_{t+1}}}", near end ]
\arrow[ llu,"\proj{\epsilon_t}" above, near end]
\arrow[ r,"\eqnFunc"]&\eqnOut(\xgusst{t})
\\
&\mathbf{xz_{in}} 
\arrow[ "gen_{\xzFuncGuess}", to=U] 
\arrow[ld,"\proj{\linMod}"]
\arrow[d,"\proj{\XZFunc}"]
\arrow[rd,"\proj{\xgusst{t}}"]
\\
\linMod&\XZFunc&\begin{bmatrix}\xgusst{t}\end{bmatrix}\\
   \end{tikzcd}
...
\arrow[rd,"\proj{\eqnFunc}"]
\\
\mathbf{dims}=\begin{bmatrix}  \numX\\ \numEps\\ \numZ\end{bmatrix} &\xzFuncGuess&\eqnFunc
   \end{tikzcd}
...
\begin{frame}[fragile]
\frametitle{do generic algorithm not}
{\small 
   ...
   \begin{tikzcd}
     \linMod \\
     \XZFunc & \arrow [ lu,"\proj{\linMod}" above ] 
alg1Args \arrow [ l, "\proj{\XZFunc}" above ]   
\arrow [ ld,"\proj{\posInt}" ]\\
     \posInt  
   \end{tikzcd}
 \begin{tikzcd}
     C \arrow [ r,"\XZFunc"] \arrow [ rd,"\xzFuncGuess"] & D \\
