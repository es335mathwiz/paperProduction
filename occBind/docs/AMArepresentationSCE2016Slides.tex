...
{\small 
   ...
   \begin{tikzcd}
     A \arrow [ r,"\xzFunc" near start,"\slvr" near end]&B 
   \end{tikzcd}
 \begin{tikzcd}
     C \arrow [ r,"\XZFunc"] \arrow [ rd,"\xzFuncGuess"] & D \\
     & B 
   \end{tikzcd}
\begin{tikzcd}
    E \arrow [ r,"\eqnFunc"]&F 
...
\begin{bmatrix}x_{t}(\xgusst{})\end{bmatrix}&
\begin{bmatrix}\expct{x_{t+1}}(\xgusst{})\end{bmatrix}\\
\begin{bmatrix}  z_t\end{bmatrix}
& 
\mathcal{I}_t 
\arrow [ u, "\proj{x_{t-1}}"right ,near end ]   
\arrow [ lu,"\proj{\epsilon_t}" above] 
\arrow[r,"\xzFuncGuess(\xgusst{})"{name=U,below}]
\arrow [ l,"\proj{z_t}" ] 
& 
\eqnArg 
...
& 
\mathcal{I}_t 
\arrow [ ldd, "\proj{x_{t-1}}"right ,near end ]   
\arrow [ ld,"\proj{\epsilon_t}" above] 
\arrow[r,"\xzFuncGuess(\xgusst{})"{name=U,below}]
\arrow [ rrdd,"\proj{z_t}", bend left=30 ] 
& 
\eqnArg 
\arrow[ lldd,"\proj{x_{t-1}}" above ]
\arrow[ rd,"\proj{x_{t}}"]
\arrow[ ru,"\proj{\expct{x_{t+1}}}", near end,crossing over ]
\arrow[ lld,"\proj{\epsilon_t}" above, near end]
\arrow[ r,"\eqnFunc"]&\mathbf{0}
\\
\begin{bmatrix}\epsilon_t\end{bmatrix}&\slvrIn
\arrow[l,"\proj{\epsilon_t}"]
\arrow[ld,"\proj{x_{t-1}}"]
\arrow[r,"\slvr"{name=bip,below}]
&\slvrOut
\arrow[r,"\proj{x_{t}(\xguss{})}"]
\arrow[rd,"\proj{z_{t}(\xguss{})}",below]
&\begin{bmatrix}x_t(\xguss{})\end{bmatrix}
\\
\begin{bmatrix}x_{t-1}\end{bmatrix}
&
\genSlvrIn 
\arrow[u,"\genSlvr",to=bip]
\arrow[ld,"\proj{\mathbf{dims}}"]
\arrow[d,"\proj{\xzFuncGuess(\xguss{})}"]
\arrow[rd,"\proj{\eqnFunc}"]&&\begin{bmatrix}z_t(\xguss{})\end{bmatrix}
\\
\mathbf{dims}=\begin{bmatrix}  \numX\\ \numEps\\ \numZ\end{bmatrix} &\xzFuncGuess(\xguss{})&\eqnFunc
   \end{tikzcd}
