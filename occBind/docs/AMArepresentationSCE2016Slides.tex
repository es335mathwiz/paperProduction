...
\arrow [ d,"\proj{z_t(\xguss)}" ,near end ] 
\arrow[r,"\xzFuncGuess(\xgusst{})"{name=U,below}]
&
\eqnArg 
%\arrow[ rd,"\proj{x_{t}}"]
...
\\
\begin{bmatrix}\epsilon_t\end{bmatrix}&
&\begin{bmatrix}z_t(\xguss{})\end{bmatrix}
&\mathbf{0}
\\
&
\genSlvrIn 
...
\arrow[rd,"\proj{\xzFuncGuess(\xguss{})}"]
\arrow[d,"\proj{\eqnFunc}"]&
\slvrOut
\arrow[r,"\proj{x_{t}(\xguss{})}"]
\arrow[u,"\proj{z_{t}(\xguss{})}" right]
&\begin{bmatrix}x_t(\xguss{})\end{bmatrix}
\\
\mathbf{dims}=\begin{bmatrix}  \numX\\ \numEps\\ \numZ\end{bmatrix} &\eqnFunc&\xzFuncGuess(\xguss{})&\xguss{}
   \end{tikzcd}
