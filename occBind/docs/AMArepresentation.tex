...
\underbrace{\xNowtp{1}=\xIter{0}{x_{t}},0}
\end{gather*}
...
It is unlikely that $g^0(x_{t-1},\epsilon_{t})$
...
\underbrace{\xNowtp{2},0=\xIter{1}{x_{t+1}}}
\end{gather*}
...
We can get a pessimistic upper bound for how far we are away from the solution 
by computing the infinity norm of substituting the variables 
\begin{gather}
\xNow{1},\xNowtp{1},\xNowtp{2}=\xIter{1}{x_{t+1}}
\end{gather}
into Equation \ref{theProblem}. If the accuracy of the approximation is adequate, we can conlude computing terms in the power series.  If not,
...
\end{gather}
 ...
 \begin{gather}
   \label{eq:1}
   x_t=B x_{t-1} + \phi z_0(x_{t-1},\epsilon_t) + F \phi \mathcal{Z}^1(x_t)\\
   x_{t+1}=B x_{t} + \phi \mathcal{Z}^1(x_t)
 \end{gather}
 ...
 \begin{gather*}
 g^{k}(x_{t-(k+1)},\epsilon_{t-k}) \intertext{ and}
 \{ z^{T-k,k}(x_{t-(k+1)},\epsilon_{t-k}),\ldots, z^{T,k}(x_{t-(k+1)},\epsilon_{t-k})\}
   \end{gather*}
  to solve for 
 \begin{gather*}
 g^{k+1}(x_{t-(k+2)},\epsilon_{t-(k+1)}) \intertext{ and}
 \{ z^{T-(k+1),k+1}(x_{t-(k+2)},\epsilon_{t-(k+1)}),\ldots, z^{T,k+1}(x_{t-(k+2)},\epsilon_{t-(k+1)})\}
   \end{gather*}
