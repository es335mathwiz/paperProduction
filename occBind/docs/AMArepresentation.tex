...
We are now in a position to easily compute 
 \begin{gather}
\xIter{0}=\mathcal{H}^{PF}[g^{0}(x,\epsilon_{t-k+1})]=
...
$z^1(x_{t-1},\epsilon_t)$ and consequently $x_t(x_{t-1},\epsilon_t)$
Consequently,
 \begin{gather}
...
$\mathcal{Z}_1(x)=\mathcal{H}^{PF}[g^{1}(x,\epsilon_{t-k+1})]$ or
$\mathcal{Z}_1(x)=\mathcal{H}^{RE}[g^{1}(x,\epsilon_{t-k+1})]$.
...
\end{gather}
 ...
 \begin{gather}
   \label{eq:1}
   x_t=B x_{t-1} + \phi z_0(x_{t-1},\epsilon_t) + F \phi \mathcal{Z}_1(x_t)\\
   x_{t+1}=B x_{t} + \phi \mathcal{Z}_1(x_t)
 \end{gather}
