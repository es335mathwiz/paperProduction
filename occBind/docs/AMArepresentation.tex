...
Consider a time invariant function $\xtFuncTI$, a function $x$ an $L$ dimensional variable and $\epsilon$ a $K$ dimensional random variable. 
In this  context
we will require
the $\epsilon_t$ to be independently and identically distributed.
We will require that
  we can also compute 
a time invariant $\XtFuncTI\equiv \expctEps{\xtFuncTI}$ with the property that
\begin{gather*}
...
\end{cases}
\end{gather*}
The function $\expctEps{\cdot}$ may be a mathematical expectation function, for ratonal expectations, but it could also be a function which assigns the value $0$ to all future $\epsilon_t$ for perfect foresight solutions.
In the text that follows, we will assume that at time $t$, $\epsilon_t$ along with all the history $x_{t-k},\, \forall k>0$ are known.
In what follows, it will become important to note that as a result, 
$\xtFunc{(x_{t-1},\epsilon)}$ and 
$\XtFunc{(\xtFunc{(x_{t-1},\epsilon)})}$ are not stochastic.  This will facilitate the solution of difficult decision problems such as problems with
...
With these conventions, it is thus possible with invariant functions to iterate the solutions forward.
If these solutions remain bounded, then there is a useful series representation
for reproducing these trajectories.
...
\subsection{Decision Rule Assessment}
