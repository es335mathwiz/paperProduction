...
\author{Gary S. Anderson\thanks{I would like to thank Luca Guerrieri, Christopher Gust and Robert Tetlow for their comments, suggestions and ecouragement. }}
\input{newCmds}
...
The series representation serves as an impotent component in an algorithm 
...
Why do we care about the things in the abstract:
\begin{itemize}
\item Time invariant functions
\item Approximation error for time invariant functions
\item Occasionally binding constraints
\item Volterra series?
\item Impulse Response functions.
\item To solve a wide class of models. 
\end{itemize}
...
We focus on models with time invariant decision rule solutions. It is difficult to characterize the model specification, but easier to specify the common characteristics of the solutions.  A s a consequence the approach is very general the text will provide a variety of models that would be amenable, but the list is not meant to be exhaustive.
...
it may prove useful to consider the associated Lagrangian.  In that case the $m$ equations and the Lagrange multiplies would be absorbed into the set of $h$ and $m$ need not eplicitly appear.
