...
\begin{itemize}
\item tool to assess any proposed model solution
\item error bounds are a novel feature of the approach
\item very general
\item very accurate  ( or not as you choose)
\item diagnostics features of model
\end{itemize}
...
\large Burnside Problem

\begin{gather}
\normalsize y_t - \beta E_t \{ (1+y_{t+1}) exp{(\theta \bar{x} + \theta \sigma \epsilon_{t+1})} \} = 0
\end {gather}

\normalsize SCHMITT-GROME (S-G) Asset Pricing Model
\begin{equation}
y_t = \beta E_t\{e^{\theta x_{t+1}} \left[ 1+y_{t+1} \right]\}
\end{equation}

\begin{equation}
x_t = (1-\rho) \bar{x} + \rho x_{t-1} + \sigma \eta \epsilon_t
\end{equation}
...
\begin{itemize}
\item zk list should increase at each iteration unless already at lengh where trunc err negligible
\item could increase by one each time  Z0 for infinity no impact different from 1, what about Z1. only one produces same ? 2,3,4 etc likely different
\item if have done upthrough l, redo l probably no impact? must go to at lesast l+1 but no need past truncation limit
\item cite holden
...
\item operation counts
\item time and space requirements
\end{itemize}
...
\begin{itemize}
\item Algorithm for computing solutions
\item Code for assessing the accuracy of proposed solutions
\item Series representation for model solutions
\item perfect foresight rational expectation
\item occasionally binding constraints
\item regime switching
\item logical variables complementary slackness
\end{itemize}
...
\begin{itemize}
\item Out of Mathematica
\item Interpolation
\item Projection Chebyshev
\item Model diagnositic assessment code  -- How difficult is the model to solve  -- 
condition number?
\item Parallelization
\item any BK model, how to choose
\item short argument list
\item compile mathematica into ``C''
\item GPU?
\item Best linear approximation?  or whatever order
\end{itemize}
