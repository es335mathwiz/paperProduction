...
can accomodate inequality constaints or altenative regimes to produce a
solution for each set of initial conditions.  One can typically arrange,
perphaps by adding auxiliary variables, to produce a ``decision rule''
...
bounded families of state vector paths generted by Mathematica code.
\begin{verbatim}
...
There is nothing in the algorithm that precludes accomodating  inequality
...
\end{gather*}
  ...
  {Iterating Stochastic Functions Forward: Conditional Expectations}
...
\end{gather}
  ...
  {An ``Almost Arbitrary'' Linear Model}
 {\small 
...
\end{gather} 
  ...
  {Series Approximation Truncation Errors}
  \begin{itemize}
...
\sum_{s=k+1}^{\infty} F^s \phi \psi_z = (I -F)^{-1} F^{k+1}\phi \psi_z       
  ...
  {An RBC example: Bound Versus Actual}
  \begin{itemize}
